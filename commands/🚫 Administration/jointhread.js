...
module.exports = {
  ...
  aliases: [`jointh`],
  description: `Make the Bot to join a Thread`,
  usage: `jointhread [#channel / Inside of a Thread]`,
  type: "thread",
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      let cmdroles = client.settings.get(message.guild.id, "cmdadminroles.jointhread")
      var cmdrole = []
      if (cmdroles.length > 0) {
        for (const r of cmdroles) {
          if (message.guild.roles.cache.get(r)) {
            ...
            //console.log(r)
            client.settings.remove(message.guild.id, r, `cmdadminroles.jointhread`)
          }
        }
      }
