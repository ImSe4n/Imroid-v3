...
module.exports = {
  ...
  aliases: [`unathread`, "unclosethread", "openthread", "othread"],
  description: `Unarchives a Thread`,
  usage: `unarchivethread [#channel / Inside of a Thread]`,
  type: "thread",
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      let cmdroles = client.settings.get(message.guild.id, "cmdadminroles.unarchivethread")
      var cmdrole = []
      if (cmdroles.length > 0) {
        for (const r of cmdroles) {
          if (message.guild.roles.cache.get(r)) {
            ...
            //console.log(r)
            client.settings.remove(message.guild.id, r, `cmdadminroles.unarchivethread`)
          }
        }
      }
      ...
      message.reply({embeds :[new MessageEmbed()
        .setColor(es.color)
        .setFooter(client.getFooter(es))
        .setTitle(`<a:yes:960701455524847667> **Successfully unarchived \`${channel.name}\`**`)
      ]});
