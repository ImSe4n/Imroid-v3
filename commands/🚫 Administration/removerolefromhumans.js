 ...
 module.exports = {
   ...
   cooldown: 60,
   usage: `removerolefromhumans @Role`,
   description: `Removes a Role from every HUMAN in this Guild`,
   type: "memberrole",
   run: async (client, message, args, cmduser, text, prefix) => {
     ...
     try {
      ...
      return message.reply({embeds : [new MessageEmbed()
       ...
       let cmdroles = client.settings.get(message.guild.id, "cmdadminroles.removerolefromhumans")
       var cmdrole = []
       if (cmdroles.length > 0) {
         for (const r of cmdroles) {
           if (message.guild.roles.cache.get(r)) {
             ...
             //console.log(r)
             client.settings.remove(message.guild.id, r, `cmdadminroles.removerolefromhumans`)
           }
         }
       }
       ...
       var members = message.guild.members.cache.filter(member => !member.user.bot && member.roles.cache.has(role.id)).map(this_Code_is_by_Sean_1751 => this_Code_is_by_Sean_1751);
       ...
       function send_finished() {
         ...
         message.reply({
           ...
           embeds: [new MessageEmbed()
             ...
             .setDescription(`Successfully removed ${role} from \`${success} HUMANS\` of \`${counter} HUMANS\`${failed != 0 ? `\n${failed} Members, did not get the Removed, redo it with: \`${prefix}removerolefromhumans ${role.id}\``: ""}`)
          ]});
       }
