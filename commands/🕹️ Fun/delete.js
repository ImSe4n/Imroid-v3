  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let es = client.settings.get(message.guild.id, "embed");let ls = client.settings.get(message.guild.id, "language")
      ...
      if(!user && args[0] && args[0].length == 18) {
        ...
        if(tmp) user = tmp;
        if(!tmp) return message.reply({content : eval(client.la[ls]["cmds"]["fun"]["delete"]["variable2"])})
      }
      else if(!user && args[0]){
        ...
        if(!user || user == null || !user.id) return message.reply({content : eval(client.la[ls]["cmds"]["fun"]["delete"]["variable3"])})
      }
      else {
      ...
      //get the memer image
      client.memer.delete(avatar).then(image => {
        //make an attachment
        var attachment = new MessageAttachment(image, "delete.png");
        //delete old message
        tempmsg.delete();
        //send new Message
