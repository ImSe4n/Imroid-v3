  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if(!user && args[0] && args[0].length == 18) {
        ...
        if(!user || user == null || !user.id) return message.reply({content : eval(client.la[ls]["cmds"]["fun"]["rip"]["variable3"])})
      }
      else {
      ...
      return message.reply({embeds : [new MessageEmbed()
        ...
        .setDescription(eval(client.la[ls]["cmds"]["fun"]["rip"]["variable4"]))
      ]});
    }
  }
}
