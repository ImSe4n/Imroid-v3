  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let es = client.settings.get(message.guild.id, "embed");let ls = client.settings.get(message.guild.id, "language")
      ...
      if(!text) return tempmsg.edit({embeds :[tempmsg.embeds[0]
        .setTitle(eval(client.la[ls]["cmds"]["fun"]["violence"]["variable2"]))
        .setColor("RED")
        .setDescription(eval(client.la[ls]["cmds"]["fun"]["violence"]["variable3"]))
      ]}).catch(() => {})
      
      //get the memer image
      client.memer.violence(text).then(image => {
        //make an attachment
        var attachment = new MessageAttachment(image, "violence.png");
        //delete old message
        tempmsg.delete();
        //send new Message
