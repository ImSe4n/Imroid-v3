  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let es = client.settings.get(message.guild.id, "embed");let ls = client.settings.get(message.guild.id, "language")
      ...
      if(!user && args[0] && args[0].length == 18) {
        ...
        if(tmp) user = tmp;
        if(!tmp) return message.reply({content : eval(client.la[ls]["cmds"]["fun"]["failure"]["variable2"])})
      }
      else if(!user && args[0]){
        ...
        if(!user || user == null || !user.id) return message.reply({content : eval(client.la[ls]["cmds"]["fun"]["failure"]["variable3"])})
      }
      else {
