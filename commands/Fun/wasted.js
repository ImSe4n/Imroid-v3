  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if(!user && args[0] && args[0].length == 18) {
        ...
        if(tmp) user = tmp;
        if(!tmp) return message.reply(eval(client.la[ls]["cmds"]["fun"]["wasted"]["variable2"]))
      }
      else if(!user && args[0]){
        ...
        if(!user || user == null || !user.id) return message.reply(eval(client.la[ls]["cmds"]["fun"]["wasted"]["variable3"]))
      }
      else {
      ...
      return message.reply({embeds : [new MessageEmbed()
        ...
        .setDescription(eval(client.la[ls]["cmds"]["fun"]["wasted"]["variable4"]))
      ]});
    }
  }
}
