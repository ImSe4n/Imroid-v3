      ...
      async function first_layer() {
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `<a:yes:955154240664371222> **Selected: \`${collected && collected.first() && collected.first().values ? collected.first().values[0] : "Nothing"}\`**`
          })
        });
      }
        ...
        switch (optionhandletype) {
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<a:no:955946429094953004> **You need to add at least 1 Open-Apply-Option**")
            }
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && collected2.first().mentions.channels.size > 0) {
                ...
                return message.reply("<a:no:955946429094953004> **You did not ping a valid Channel!**")
              }
            } else {
              return message.reply("<a:no:955946429094953004> **You did not enter a Valid Message in Time! CANCELLED!**")
            }
          }
          break;
          case "Add Apply Option": {
            let data = theDB.get(message.guild.id, pre+".data");
            if (data.length >= 25) {
              return message.reply("<a:no:955946429094953004> **You reached the limit of 25 different Options!** Remove another Option first!")
            }
            ...
            if (collected && collected.first().content) {
              if (!collected.first().content.includes("++")) return message.reply("<a:no:955946429094953004> **Invalid Usage! Please mind the Usage and check the Example**")
              let value = collected.first().content.split("++")[0].trim().substring(0, 25);
              let index = data.findIndex(v => v.value == value);
              if(index >= 0) {
                  return message.reply("<a:no:955946429094953004> **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
              ...
              collector.on('collect', menu => {
                if (menu?.user.id === cmduser.id) {
                  ...
                  if(index >= 0) {
                      return message.reply("<a:no:955946429094953004> **Options can't start the Same Apply System!** There is already an Option with that Application System!");
                  }
                  ...
                  if(!apply_for_here.has(message.guild.id) || !apply_for_here.has(message.guild.id, applypre) || !apply_for_here.has(message.guild.id, applypre+".QUESTIONS") || apply_for_here.get(message.guild.id, applypre+".QUESTIONS").length < 1) 
                    return message.reply(`<a:no:955946429094953004> **The ${applySystemExecution}. Apply System is not setupped / has no Questions, create it first with: \`${prefix}setup-apply\`**`)
          ...
          let data = theDB.get(message.guild.id, pre+".data");
          if (!data || data.length < 1) {
            return message.reply("<a:no:955946429094953004> **There are no Open-Apply-Options to remove**")
          }
          let embed = new MessageEmbed()
            .setColor(es.color)
            .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          ...
          collector.on('end', collected => {
            menumsg.edit({
              embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
              components: [],
              content: `<a:yes:955154240664371222> **Selected: \`${collected.first().values[0]}\`**`
            })
          });
        }
        break;
