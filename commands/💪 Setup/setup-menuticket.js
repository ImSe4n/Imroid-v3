      ...
      async function first_layer() {
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `<:yes:> **Selected: \`${collected && collected.first() && collected.first().values ? collected.first().values[0] : "Nothing"}\`**`
          }).catch(() => { });
        });
      }
        ...
        switch (optionhandletype) {
          ...
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:no:> **You need to add at least 1 Open-Ticket-Option**")
            }
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && (collected2.first().mentions.channels.size > 0 || message.guild.channels.cache.get(collected2.first().content?.trim()))) {
                ...
                return message.reply("<:no:> **You did not ping a valid Channel!**")
              }
            } else {
              return message.reply("<:no:> **You did not enter a Valid Message in Time! CANCELLED!**")
            }
          }
            break;
          case "Add Ticket Option": {
            let data = theDB.get(message.guild.id, pre + ".data");
            if (data.length >= 25) {
              return message.reply("<:no:> **You reached the limit of 25 different Options!** Remove another Option first!")
            }
            ...
            if (collected && collected.first().content) {
              if (!collected.first().content.includes("++")) return message.reply("<:no:> **Invalid Usage! Please mind the Usage and check the Example**")
              let value = collected.first().content.split("++")[0].trim().substring(0, 25);
              let index = data.findIndex(v => v.value == value);
              if (index >= 0) {
                return message.reply("<:no:> **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
          ...
          case "Edit Ticket Option": {
            ...
            let data = theDB.get(message.guild.id, pre + ".data");
            if (!data || data.length < 1) {
              return message.reply("<:no:> **There are no Open-Ticket-Options to remove**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                ...
                async function handle_the_picks3(optionhandletype) {
                  ...
                  switch (optionhandletype) {
                    ...
                    case `Change Open Category`: {
                      ...
                      return message.reply("<:no:> **Invalid Category-ID added**")
                    } break;
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<:yes:> **Selected: \`${collected.size > 0 ? collected.first().values[0] : "NOTHING"}\`**`
              })
            });
          }
            break;
          case "Remove Ticket Option": {
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<:yes:> **Selected: \`${collected.first().values[0]}\`**`
              })
            });
          }
            break;
