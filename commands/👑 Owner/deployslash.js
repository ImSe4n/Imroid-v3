    ...
    if (message.author.id != "711602696133673011")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only Sean is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://dsc.gg/imroid-development), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:Lwoading:964690249810862080> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:Lwoading:964690249810862080> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
