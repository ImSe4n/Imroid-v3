 ...
 module.exports = {
   ...
   description: `Adds a Role to every HUMAN in this Guild`,
   type: "memberrole",
   run: async (client, message, args, cmduser, text, prefix) => {
     ...
     try {
      ...
      return message.reply({embeds : [new MessageEmbed()
       ...
       let cmdroles = client.settings.get(message.guild.id, "cmdadminroles.addroletohumans")
       var cmdrole = []
       if (cmdroles.length > 0) {
         for (const r of cmdroles) {
           if (message.guild.roles.cache.get(r)) {
             ...
             //console.log(r)
             client.settings.remove(message.guild.id, r, `cmdadminroles.addroletohumans`)
           }
         }
       }
