...
module.exports = {
  ...
  aliases: [`leaveth`],
  description: `Make the Bot to Leave a Thread`,
  usage: `leavethread [#channel / Inside of a Thread]`,
  type: "thread",
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      let cmdroles = client.settings.get(message.guild.id, "cmdadminroles.leavethread")
      var cmdrole = []
      if (cmdroles.length > 0) {
        for (const r of cmdroles) {
          if (message.guild.roles.cache.get(r)) {
            ...
            //console.log(r)
            client.settings.remove(message.guild.id, r, `cmdadminroles.leavethread`)
          }
        }
      }
      ...
      if(!channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:nah:960701828675297300> **\`${channel.name}\` Channel is not a Thread**`)
        ]});
      await channel.leave().then(()=>{
        if(channel.id != message.channel.id){
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:yes:960701455524847667> **I left \`${channel.name}\`**`)
          ]});
        }
      }).catch(()=>{
        message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:yes:960701455524847667> **I couldn't leave \`${channel.name}\`**`)
        ]});
      })
